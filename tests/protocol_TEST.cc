...
TEST(ProtocolTests, ParsePingStatus) {
    ...
    uint8_t expected[] = {0x06, 0x04, 0x26};
    ...
    ASSERT_EQ(result, DYNAMIXEL_ERROR_NONE);
    ASSERT_EQ(param_length, sizeof(expected));

    ASSERT_EQ(header.header[0], 0xFF);
    ASSERT_EQ(header.header[1], 0xFF);
    ASSERT_EQ(header.header[2], 0xFD);
    ASSERT_EQ(header.reserved, 0x0);
    ASSERT_EQ(header.id, 0x01);
    ASSERT_EQ(header.length, 7);
    ASSERT_EQ(header.instruction, STATUS);
    ASSERT_EQ(header.error, STATUS_OK);

    ASSERT_EQ(memcmp(param, expected, sizeof(expected)), 0);
}

TEST(ProtocolTests, ParseCRCFail) {
    uint8_t status_packet[] = {
        0xFF, 0xFF, 0xFD, 0x00,
        0x01, 0x07, 0x00, 0x55,
        0x00, 0x06, 0x04, 0x26,
    ...
    ASSERT_EQ(result, DYNAMIXEL_ERROR_CRC_FAIL);
}

TEST(ProtocolTests, ParseNoParamSpace) {
    uint8_t status_packet[] = {
        0xFF, 0xFF, 0xFD, 0x00,
        0x01, 0x07, 0x00, 0x55,
        0x00, 0x06, 0x04, 0x26,
